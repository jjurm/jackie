...
public class ObstacleAvoidanceState extends AbstractState {
	...
	public State run0() {
		// int side =
		// (robot.commander.readUltrasonicSensor(Commander.ULTRASONIC_LEFT) >
		// robot.commander.readUltrasonicSensor(Commander.ULTRASONIC_RIGHT)) ? 1
		// : -1;
		int side = 1;
		...
		Triple<Point, Point, Point> p = robot.lineFollowingState.strategy
				.checkLine(new Point(img.getWidth() / 2, img.getHeight() - 10), -Math.PI / 2);
		if (p != null) {
